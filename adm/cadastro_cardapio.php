...
function redimensiona($origem,$destino,$maxlargura,$maxaltura,$qualidade){
	...
	if (($_GET["id"]!=null)){
		$sql	= "SELECT id,imagem,titulo,conteudo FROM cardapio where (id=0".$_GET["id"].")";
		$result=mysql_query($sql, $link);
...
<div id="individual_cadastro">
	<div id="nome_cadastro">CADASTRO - CARDÁPIO</div>
		...
		if ($_POST) {
			...
			)
			{
				if (($_FILES["imagem"]["size"] < 5*1024*1024)){
					...
					else
					{
						...
						redimensiona($folder . $imagem,$folder."g_".$imagem,79,52,75);
						
						unlink($folder . $imagem);
						//delete_file($folder . $imagem);	
						echo "<a href=\"../uploads/cardapio/g_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
					}
				}
				else
				{
			...
			if($result!=null){
				...
				$sql = "insert into cardapio (imagem,titulo,conteudo) values ('".$imagem."','".$_POST["titulo"]."','".$conteudo."');";
				if($sql){
				...
				if ($_POST['acao']=='Excluir'){
					...
					$conteudo=($_POST["conteudo"]);
					$conteudo=str_replace("\\\"", "\"", $conteudo);
					$sql = "update cardapio set imagem='".$imagem."',titulo='".$_POST["titulo"]."',conteudo='".$conteudo."' where (id=".$_POST["id"].");";
					//echo $sql;
					if($sql){
	  ...
	  <div id="relatorio_input" >
		<table cellpadding= "7" cellspacing = "0" border= "1" width="1004">
			...
			?>
				<tr>
					<td>
						<a href="?pg=cadastro_cardapio&id=<?php echo $row['id'];?>">
					...
					<td>
						<img width="60px" height="60px" src="../uploads/cardapio/g_<?php echo $row['imagem'];?>">
					</td>
				</tr>
			<?php
				}
