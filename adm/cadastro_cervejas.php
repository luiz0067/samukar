...
function redimensiona($origem,$destino,$maxlargura,$maxaltura,$qualidade){
	...
	if (($_GET["id"]!=null)){
		$sql	= "SELECT id,imagem,obs FROM cervejas where (id=0".$_GET["id"].")";
		$result=mysql_query($sql, $link);
		...
		$agenda=0;
		if (isset($_GET["cervejas"])){
		$cervejas=$_GET["cervejas"];	
	}
		else if (isset($_POST["cervejas"])){
		$cervejas=$_POST["cervejas"];
	}	 
?>


<div id="individual_cadastro">
	<div id="nome_cadastro">CADASTRO - CERVEJAS</div>
		...
		if ($_POST) {
			...
			)
			{
				if (($_FILES["imagem"]["size"] < 5*1024*1024)){
					...
					else
					{
						...
						redimensiona($folder . $imagem,$folder."g_".$imagem,150,72,75);
						
						unlink($folder . $imagem);
						//delete_file($folder . $imagem);	
						echo "<a href=\"../uploads/cervejas/g_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
					}
				}
				else
				{
			...
			if($result!=null){
				...
				$sql = "insert into cervejas (imagem,obs) values ('".$imagem."','".$_POST["obs"]."');";
				echo $sql;
				if($sql){
				...
				if ($_POST['acao']=='Excluir'){
					$sql = 'delete FROM cervejas where id='.$_POST["id"];
					   if($sql){
					...
					$sql = "update cervejas set imagem='".$imagem."',obs='".$_POST["obs"]."' where (id=".$_POST["id"].");";
					//echo $sql;
					if($sql){
	  ...
	  <div id="relatorio_input" >
		<table cellpadding= "7" cellspacing = "0" border= "1" width="1004">
			...
			?>
				<tr>
					<td>
						<a href="?pg=cadastro_cervejas&id=<?php echo $row['id'];?>">
					...
					<td><p><?php echo $row['obs'];?></p> </td>
					<td>
						<img width="150px" height="72px" src="../uploads/cervejas/g_<?php echo $row['imagem'];?>">
					</td>			
				</tr>
			<?php
				}
