...
function redimensiona($origem,$destino,$maxlargura,$maxaltura,$qualidade){
	...
	if (($_GET["id"]!=null)){
		$sql	= "SELECT id,imagem,titulo FROM bar where (id=0".$_GET["id"].")";
		$result=mysql_query($sql, $link);
...
<div id="individual_cadastro">
	<div id="nome_cadastro">CADASTRO - FOTOS DO BAR</div>
		...
		if ($_POST) {
			...
			)
			{
				if (($_FILES["imagem"]["size"] < 5*1024*1024)){
					...
					else
					{
						...
						redimensiona($folder . $imagem,$folder."g_".$imagem,138,92,75);
						
						unlink($folder . $imagem);
						//delete_file($folder . $imagem);	
						echo "<a href=\"../uploads/bar/g_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
					}
				}
				else
				{
			...
			if($result!=null){
				...
				$sql = "insert into bar (imagem,titulo) values ('".$imagem."','".$_POST["titulo"]."');";
				echo $sql;
				if($sql){
				...
				if ($_POST['acao']=='Excluir'){
					$sql = 'delete FROM bar where id='.$_POST["id"];
					   if($sql){
					...
					$conteudo=($_POST["conteudo"]);
					$conteudo=str_replace("\\\"", "\"", $conteudo);
					$sql = "update bar set imagem='".$imagem."',titulo='".$_POST["titulo"]."' where (id=".$_POST["id"].");";
					//echo $sql;
					if($sql){
	  ...
	  <div id="relatorio_input" >
		<table cellpadding= "7" cellspacing = "0" border= "1" width="1004">
			...
			$sql    = 'SELECT id,imagem,titulo FROM bar;';
			$result = mysql_query($sql, $link);
			if (!$result) {
			...
			?>
				<tr>
					<td>
						<a href="?pg=cadastro_fotos_bar&id=<?php echo $row['id'];?>">
					...
					<td>
						<img width="60px" height="60px" src="../uploads/bar/g_<?php echo $row['imagem'];?>">
					</td>
				</tr>
			<?php
				}
