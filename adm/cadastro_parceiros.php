...
function redimensiona($origem,$destino,$maxlargura,$maxaltura,$qualidade){
	...
	if (($_GET["id"]!=null)){
		$sql	= "SELECT id,imagem,data_evento,titulo,link FROM parceiros where (id=0".$_GET["id"].")";
		$result=mysql_query($sql, $link);
		...
		$parceiros=0;
		if (isset($_GET["parceiros"])){
		$parceiros=$_GET["parceiros"];	
	}
		else if (isset($_POST["parceiros"])){
		$parceiros=$_POST["parceiros"];
	}	 

?>
		<div id="individual_cadastro">
			<div id="nome_cadastro">CADASTRO - PARCEIROS</div>
					...
					<form method="post" enctype="multipart/form-data" name="form1" id="form1">
						<div id="inputs_formularios">
							...
							<div class="tudo_input">
								...
								<div class="left_input">
									<input name="link" style="width:530px;" type="text" value="<?php echo $row["link"]?>">
								</div>
								<div class="clear"></div>
							</div>
					...
					<?php
							if ($_POST) {
								...
								)
								{
									if (($_FILES["imagem"]["size"] < 5*1024*1024)){
										...
										else
										{
											...
											redimensiona($folder . $imagem,$folder."h_".$imagem,165,90,75);
											redimensiona($folder . $imagem,$folder."g_".$imagem,580,300,75);
											
											unlink($folder . $imagem);
											//delete_file($folder . $imagem);	
											echo "<a href=\"../uploads/parceiros/g_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
										}
									}
									else
									{
								...
								if($result!=null){
									...
									$_POST["data_evento"]=Date("Y-m-d H:i:s");    				
									$sql = "insert into parceiros (imagem,data_evento,titulo,link) values ('".$imagem."','".$_POST["data_evento"]."','".$_POST["titulo"]."','".$_POST["link"]."');";
									//echo $sql;
									if($sql){
									...
									if ($_POST['acao']=='Excluir'){
										$sql = 'delete FROM parceiros where id='.$_POST["id"];
										   if($sql){
									...
									$_POST["data_evento"]=Date("Y-m-d H:i:s"); 
										$sql = "update parceiros set imagem='".$imagem."',titulo='".$_POST["titulo"]."',data_evento='".$_POST["data_evento"]."',link='".$_POST["link"]."' where (id=".$_POST["id"].");";
										//echo $sql;										
										if($sql){
			  ...
			  <div id="relatorio_input" >
				<table cellpadding= "7" cellspacing = "0" border= "1" width="1004">
					...
					?>
						<tr>
							<td>
								<a href="?pg=cadastro_parceiros&id=<?php echo $row['id'];?>">
							...
							<td><p><?php echo $row['link'];?></p></td>
							<td>
								<img width="60px" height="60px" src="../uploads/parceiros/g_<?php echo $row['imagem'];?>">
							</td>
						</tr>
					<?php
						}
